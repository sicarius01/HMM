...
class HMM:
    ...
    ----------------------------
    A : numpy.ndarray
        State transition probability matrix with shape(N, N)
    B : numpy.ndarray
    ...
    def state_path(self, obs_seq: list[int]) -> tuple[float, list[int]]:
        """
        Returns
        -----------
    ...
    def viterbi(self, obs_seq: list[int]) -> tuple[np.ndarray, np.ndarray]:
        """
        Returns
        -----------
        V : numpy.ndarray
        ...
        N: int = self.A.shape[0]
        T: int = len(obs_seq)
        prev: np.ndarray = np.zeros((T - 1, N), dtype=np.int64)
    ...
    def build_viterbi_path(self, prev: list[int], last_state: int):
    ...
    def baum_welch_train(self, obs_seq: list[int]) -> None:
        N: int = self.A.shape[0]
        T = len(obs_seq)

        self.forw: np.ndarray = self._forward(obs_seq)
        self.back: np.ndarray = self._backward(obs_seq)
        forw = self.forw
        back = self.back

        # P( entire observation sequence | A, B, pi )
        obs_prob: float = np.sum(forw[:, -1])
        if obs_prob <= 0:
            raise ValueError("P(O | lambda) = 0. Cannot optimize!")
        
        xi: np.ndarray = np.zeros((T - 1, N, N))
